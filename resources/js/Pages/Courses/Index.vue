  ...
  <app-layout>
    ...
    <section class="py-6">
      <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
          ...
          <div class="mx-12 bg-white rounded shadow p-4">
            ...
            >
              ...
              <span class="block text-sm text-gray-400">
                {{ course.episodes_count }} épisode<span
                  v-if="course.episodes_count > 1"
                  >s</span
                >
              </span>
            </div>
            ...
            <div class="flex items-center justify-between">
              ...
              <a
                ...
                v-if="course.update"
                class="bg-gray-500 text-white px-2 py-1 text-sm mt-3 inline-block rounded hover:bg-gray-700"
        ...
        <inertia-link
          :href="link.url"
          class="text-indigo-600 border-gray-500 p-5"
          v-for="link in courses.links"
          v-bind:key="link.label"
        >
          <span v-bind:class="{'bg-indigo-900 py-1 px-3 rounded-lg text-white' : link.active}">{{ link.label }}</span>
        </inertia-link>
      </div>
    </section>
  </app-layout>
